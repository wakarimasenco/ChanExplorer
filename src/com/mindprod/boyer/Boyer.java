 ...
 * <p/>
 * String.indexOf is particularly slow when the pattern begins with a common letter such as &quot;e&quot;.
 * Boyer-Moore is fastest
 * when the pattern is long and composed only of uncommon letters, e.g. &quot;z&quot; or &quot;^&quot;. If you use a
 * char[] instead of
 * String for your text to be searched, it will run an additional 33% faster.
 * <p/>
...
public final class Boyer
    ...
    /**
     * constructor that also sets text to search in for subsequent indexOf searches. Pattern provided later with
     * indexOf.     * @param text char array to search in. may be 0-length but not null.
     *
     * @noinspection WeakerAccess
     */
    ...
    private int indexOfViaText( int index )
        ...
        if ( ( lenText <= TEXTBREAKEVENLENGTH / 2
               || lenPat <= PATTERNBREAKEVENLENGTH ) )
            {
        ...
        outer:
        for ( int tforward = index + lenPat - 1; tforward < lenText; tforward +=
                skip[ testChar & 0xff ] )
            {
            ...
            for ( int tback = tforward - 1, pback = lenPat - 2; pback
                                                                >= 0; tback--, pback-- )
                {
    ...
    public static void main( String[] args )
        ...
        if ( DEBUGGING )
            ...
            System.out.println( Boyer.indexOf( "crtcamccmcarogcatwombat",
                    "cat" ) );
            ...
            String s =
                    "create table foo (created_date datetime default sysdate not null)";
